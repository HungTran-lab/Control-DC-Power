...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _locate_mode_volt_file(self):
        """Find the external mapping CSV file (kept OUTSIDE the exe so you can edit/add).
        ...
        Accept file names (any one is OK):
          - model_vold.csv  (your preferred name)
          - model_volt.csv
          - mode_volt.csv
          - (and the same names without .csv)
        """
        base_dirs = []
        try:
    ...
    def load_mode_volt_map(self):
        ...
        if not path:
            # Create an external template beside exe/script (so you can keep editing it)
            try:
                if getattr(sys, "frozen", False):
            ...
            # Prefer your filename; user can rename freely (we accept several names)
            template_path = os.path.join(app_dir, "model_vold.csv")
            ...
            try:
                self.log("Mapping CSV not found -> created template 'model_vold.csv' beside app; mapping disabled until you fill it")
            except Exception:
                pass
            return
    ...
    def on_scanner_data(self, text: str):
        ...
        Update: chỉ lấy 4 ký tự đầu tiên sau khi scan.
        """
        text4 = (text or "").strip()[:4]
        if getattr(self, 'lbl_scanner', None):
            self.lbl_scanner.setText(text4)
        try:
            self.apply_scanner_vi(text4)
        except Exception as e:
